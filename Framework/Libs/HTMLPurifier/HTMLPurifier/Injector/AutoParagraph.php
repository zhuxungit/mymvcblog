...
/**
 * Injector that auto paragraphs text.xml in the root node based on
 * double-spacing.
...
class HTMLPurifier_Injector_AutoParagraph extends HTMLPurifier_Injector
{
    ...
    public function handleText(&$token)
    {
        ...
        if ($this->allowsElement('p')) {
            if (empty($this->currentNesting) || strpos($text, "\n\n") !== false) {
                // Note that we have differing behavior when dealing with text.xml
                // in the anonymous root node, or a node inside the document.
                // If the text.xml as a double-newline, the treatment is the same;
    ...
    /**
     * Splits up a text.xml in paragraph tokens and appends them
     * to the result stream that will replace the original
     * @param string $data String text.xml data that will be processed
     *    into paragraphs
